...
(() => {
 ...
 function stop(){clearTimeout(timer);frame.removeAttribute('src');stage.hidden=true;launch.hidden=false;actions.hidden=true;status.textContent='Loads when you press play.';}
 function start(){clearTimeout(timer);launch.hidden=true;stage.hidden=false;actions.hidden=false;status.textContent='Downloading Schism… this may take a moment.';frame.src=url.href;timer=setTimeout(()=>{status.textContent='Still waiting for the game. Check your connection, restart, or close the game and try again.';},90000);}
 launch.addEventListener('click',start);
 shell.querySelector('[data-restart]').addEventListener('click',()=>{if(confirm('Restart Schism? Current progress will be lost.'))start();});
 ...
 window.addEventListener('message',event=>{if(event.source!==frame.contentWindow||event.origin!==url.origin)return;const data=event.data;if(!data||data.game!=='schism')return;if(data.state==='volume'){audioStatus.textContent=data.value===0?'Game audio muted':'Game volume updated';return;}if(data.state==='ready'){sendVolume();clearTimeout(timer);status.textContent=matchMedia('(any-pointer: coarse)').matches?'Use the two touch pads. Up jumps; down interacts.':'Click game to use keyboard.';frame.focus();}else if(data.state==='error'){clearTimeout(timer);status.textContent='The game could not start. Try restarting or use a browser with WebGL 2 support.';}else if(data.state==='progress'){status.textContent='Downloading Schism… '+Math.min(100,Math.max(0,Number(data.percent)||0))+'%';}});
