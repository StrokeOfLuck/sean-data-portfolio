...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .page h1 { font-size:42px; margin:0 0 16px; }
  ...
  function geom() {
    ...
    if(window.matchMedia('(min-width:761px)').matches) {
      ...
      const headerBottom=header
        ...
        : 52;
      // A bike sitting on a flat deck extends roughly 47 px above the
      // track surface. Reserve a little extra room so no bike style can hide
      // behind the fixed navbar, even on very wide / short displays.
      const bikeTopClearance=58;
      const minLipY=headerBottom+bikeTopClearance;
      const bottomClearance=64;
      const minDeck=clamp(W*0.035,42,84);
      const maxRadiusX=Math.max(105,center-flatHalf-minDeck);
      const maxRadiusY=Math.max(105,(H-bottomClearance)-minLipY);
