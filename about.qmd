...
(() => {
  ...
  function geom() {
    ...
    const bottom=Math.max(headerBottom+120,H-bottomClearance);
    ...
    // If there is not enough horizontal room to route the curves around the
    // content, put the entire ramp below it instead.
    const bikeTopClearance=mobile ? 46 : 58;
    let minLipY=headerBottom+bikeTopClearance;
    if(!canWrapContent) {
      minLipY=Math.max(minLipY,content.bottom+(mobile ? 14 : 22));
    }

    const maxRadiusX=Math.max(28,center-flatHalf-minDeck);
    const maxRadiusY=Math.max(28,bottom-minLipY);
    const R=Math.max(28,Math.min(maxRadiusX,maxRadiusY));
    const lipY=bottom-R;
  ...
  function resize() {
    ...
    if(!bike.airborne && W) {
      const g=geom();
      if(wasLeftDeck) {
        bike.x=safeDeckX(g,'left');
      } else if(wasRightDeck) {
        bike.x=safeDeckX(g,'right');
      } else {
        bike.x=clamp(bike.x||safeDeckX(g,'left'),20,W-20);
      }
