...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .page h1 { font-size:42px; margin:0 0 20px; }
...
(() => {
  ...
  function geom() {
    const center=W/2;
    const flatHalf=clamp(W*0.055,34,58);
    const blendLenBase=28;

    if(window.matchMedia('(min-width:761px)').matches) {
      // Desktop: fit the *entire* halfpipe inside the viewport every time.
      // Reserve visible deck space on both sides, room below for the HUD,
      // and clearance below the site header. Ultrawide screens therefore
      // make the ramp wider/taller instead of pushing its lips off-screen.
      const header=document.getElementById('quarto-header');
      const headerBottom=header
        ? header.getBoundingClientRect().bottom
        : 52;
      const topClearance=clamp(headerBottom+14,58,150);
      const bottomClearance=64;
      const minDeck=clamp(W*0.035,42,84);
      const maxRadiusX=Math.max(105,center-flatHalf-minDeck);
      const maxRadiusY=Math.max(105,H-bottomClearance-topClearance);
      const R=Math.max(105,Math.min(maxRadiusX,maxRadiusY));
      const bottom=H-bottomClearance;
      const lipY=bottom-R;
      ...
      const blendLen=Math.min(blendLenBase,Math.max(16,R*.09));
    ...
    // Mobile keeps a smaller document-based ramp because the About page is
    // intentionally scrollable on narrow screens.
