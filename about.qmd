...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .page {
    ...
    max-width:1100px;
    margin:0 auto;
    padding:46px 32px 690px;
  }

  /* Desktop playground is a single-screen scene. The page itself should not
     scroll; only the physics objects move. Mobile keeps normal document
     scrolling so the About copy is not clipped on a narrow screen. */
  ...
  .page h1 { font-size:42px; margin:0 0 28px; }
  ...
  body.page-locked .physics-divider {
    cursor:default;
  }
  ...
  .telemetry {
    ...
    box-shadow:0 5px 20px rgba(28,42,58,.055);
  ...
  .telemetry-records-four {
    grid-template-columns:repeat(4,minmax(0,1fr));
    gap:8px 12px;
  }

  .telemetry-records-two {
    grid-template-columns:repeat(2,minmax(0,1fr));
    gap:10px 12px;
  }

  .telemetry-records strong {
    display:block;
    font-size:12px;
    line-height:1.15;
    font-weight:600;
  ...
  .telemetry-raw-pairs > div {
    min-width:0;
  }

  .telemetry-raw-pairs span {
    display:block;
    margin-bottom:2px;
    font-size:8px;
    letter-spacing:.08em;
  ...
  .telemetry-pair strong:last-child {
    margin-bottom:0;
  }
  ...
  @media(max-width:760px) {
    ...
    .photo-wrap { width:min(100%,340px); height:auto; aspect-ratio:1; }
    .interests { grid-template-columns:1fr; gap:12px; }
    .page { padding-bottom:660px; }
    .hint { display:none; }
    .telemetry {
      left:8px;
      bottom:60px;
  ...
  #quarto-header, .nav-footer { position:relative; z-index:50; }
...
<div class="page">
  ...
  <div class="physics-divider" id="interestsDivider"></div>
...
<div class="telemetry" aria-label="Live ride telemetry">
...
(() => {
  ...
  function pageSize() {
    ...
    // Desktop is deliberately one screen with no document scrolling.
    if(window.matchMedia('(min-width:761px)').matches) {
      return {w:viewportW,h:viewportH};
    }

    const pageBottom=pageEl
      ? pageEl.offsetTop + pageEl.offsetHeight
      : viewportH;

    return {
      w:viewportW,
      h:Math.max(viewportH,pageBottom + 24)
    };
  }

  function geom() {
    const bottom=trackBottomY || Math.max(420,H-350);
    const center=W/2;

    // Keep the halfpipe itself a consistent size on normal desktop widths.
    // Only shrink it when the viewport is too narrow to fit safely.
    const sideMargin=32;
    const flatHalf=clamp(W*0.06, 34, 55);
    const maxRadius=Math.max(105,(W-(sideMargin*2)-(flatHalf*2))/2);
    const R=Math.min(260,maxRadius);
    const blendLen=Math.min(28,Math.max(16,R*.09));
  ...
  function resize() {
    ...
    // Anchor the halfpipe just below the actual About content instead of to
    // the document's overflow height. This keeps the platforms, lips and
    // bottom of the ramp visible and prevents off-screen drift.
    const contentBottom=(pageEl && interestsEl)
      ? pageEl.offsetTop + interestsEl.offsetTop + interestsEl.offsetHeight
      : Math.max(260,(window.innerHeight || 0)*.48);
    trackBottomY=clamp(
      Math.max(contentBottom + 165,(window.innerHeight || 0)-105),
      420,
      Math.max(420,H-80)
    );
    ...
    if(!bike.airborne && W) {
      bike.x=clamp(bike.x||W/2,20,W-20);
  ...
  function wordBody(span) {
    ...
    return {
      ...
      y:r.top+window.scrollY-cameraShiftY+r.height/2,
  ...
  function detachDivider(vx=0,vy=0) {
    const divider=document.getElementById('interestsDivider');
    if(pageLocked || !divider || divider.dataset.detached==='1') return null;
    const b=dividerBody(divider);
    divider.dataset.detached='1';
  ...
  function restoreWordsAndPhoto() {
    ...
    // Clean up copies created by older deployed versions too.
    document.querySelectorAll('.physics-floating-word').forEach(el=>el.remove());
    bodies.length=0;
    document.querySelectorAll('.physics-word').forEach(s=>{ s.dataset.detached=''; s.style.visibility=''; });
    const divider=document.getElementById('interestsDivider');
    if(divider) {
      divider.dataset.detached='';
  ...
  function setPageLocked(next) {
    ...
    if(pageLocked) {
      ...
      pageLockButton.title='Enable falling page objects';
      pageLockButton.setAttribute('aria-label','Zen mode on. Enable falling page objects.');
      pageLockButton.classList.add('active');
      if(zenNote) zenNote.style.display='block';
      if(tLoose) tLoose.textContent='OFF';
      if(tImpacts) tImpacts.textContent='OFF';
    } else {
      pageLockButton.textContent='Zen mode off';
      pageLockButton.title='Disable falling page objects';
      pageLockButton.setAttribute('aria-label','Zen mode off. Disable falling page objects.');
  ...
  function drawTrack() {
    ...
    ctx.fillStyle='rgba(220,227,235,.24)';
    ctx.beginPath();
    ctx.moveTo(g.leftLip,g.lipY);
    for(let x=g.leftLip;x<=g.leftFlat;x+=3) ctx.lineTo(x,trackY(x));
    ctx.lineTo(g.leftFlat,g.bottom+2);
    ctx.lineTo(g.leftLip,g.bottom+2);
    ctx.closePath();
    ctx.fill();

    ctx.beginPath();
    ctx.moveTo(g.rightFlat,g.bottom+2);
    for(let x=g.rightFlat;x<=g.rightLip;x+=3) ctx.lineTo(x,trackY(x));
    ctx.lineTo(g.rightLip,g.bottom+2);
    ctx.closePath();
    ctx.fill();
  ...
  function drawBike() {
    ...
    // Fill wheel interiors first so the ramp line does not visually run
    // through the wheels and make the contact point look inconsistent.
  ...
  function hitStaticRect(x,y,r,rect) {
    ...
    const top=rect.top+window.scrollY-cameraShiftY;
    const bottom=rect.bottom+window.scrollY-cameraShiftY;
  ...
  function bikeKnockStatic() {
    if(pageLocked || performance.now()<resetGuardUntil) return;
    const r=26;
    ...
    const divider=document.getElementById('interestsDivider');
    if(divider && divider.dataset.detached!=='1' && hitStaticRect(bike.x,bike.y,r+8,divider.getBoundingClientRect())) {
      impacts+=1;
      resetStreakState();
      bike.comboBoost*=.65;
      const d=detachDivider(
        bike.airborne ? bike.vx*.55 : bike.speed*.72,
        bike.airborne ? bike.vy*.22-1.2 : -2.0
      );
      if(d) d.av=(Math.random()-.5)*.02;
    }
  }
  ...
  function movingBodyKnockStatic(body) {
    ...
    const divider=document.getElementById('interestsDivider');
    if(divider && divider.dataset.detached!=='1' && body.type!=='divider' && hitStaticRect(body.x,body.y,r+10,divider.getBoundingClientRect())) {
      const d=detachDivider(body.vx*.60, body.vy*.36-0.9);
      if(d) {
        body.vx*=.78;
        body.vy*=.84;
      }
  ...
  function drawBodies() {
    ...
    for(const b of bodies) {
      ...
      if(b.type==='word') {
        ...
        // Render the divider as a simple rigid beam, not as a stretched photo.
        ctx.fillStyle=b.color || '#dfe4ea';
        ctx.fillRect(-b.w/2,-1,b.w,2);
      } else if(b.type==='photo' && photoReady) {
        const x=-b.w/2, y=-b.h/2, w=b.w, h=b.h, r=16;
        ctx.beginPath();
        ctx.moveTo(x+r,y);
        ...
        ctx.closePath();
        ctx.clip();
        ctx.drawImage(photoBitmap,-b.w/2,-b.h/2,b.w,b.h);
      }
      ctx.restore();
    }
  }
  ...
  function eventPagePos(e) {
    return {
      ...
      y:e.clientY+window.scrollY-cameraShiftY
    };
  }
  ...
  function onPendingStaticMove(e) {
    ...
    if(dist>5) {
      ...
      else if(pendingStatic.kind==='divider') body=detachDivider();
  ...
  function onPendingStaticUp(e) {
    ...
    } else if(pendingStatic.kind==='divider') {
      detachDivider((Math.random()-.5)*.7,-.9-Math.random()*.5);
    }
  ...
  canvas.addEventListener('pointerdown',e=>{
    ...
    const divider=under?.closest?.('#interestsDivider');
    if(divider && divider.dataset.detached!=='1' && !pageLocked) {
      pendingStatic={
        kind:'divider',
        target:divider,
        startClientX:e.clientX,
...
(() => {
  ...
  function pageSize() {
    return {
      w:Math.max(document.documentElement.clientWidth,document.documentElement.scrollWidth),
  ...
  function geom() {
    const bottom=H-350;
    const center=W/2;
    const deckWidth=clamp(W*0.05, 38, 82);
    const flatHalf=clamp(W*0.025, 34, 58);
    const R=Math.max(220, center-flatHalf-deckWidth);
    const blendLen=clamp(W*0.02, 18, 34);

    const leftFlat=center-flatHalf;
    const rightFlat=center+flatHalf;
    const leftLip=deckWidth;
    const rightLip=W-deckWidth;
  ...
  function drawTrack() {
    ...
    ctx.fillStyle='rgba(220,227,235,.15)';
    ctx.beginPath();
    ctx.moveTo(g.leftLip,g.lipY);
    for(let x=g.leftLip;x<=g.leftFlat;x+=3) ctx.lineTo(x,trackY(x));
    ctx.lineTo(g.leftFlat,g.bottom+2);
