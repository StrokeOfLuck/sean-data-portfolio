...
(() => {
  ...
  function pageSize() {
    // Never use scrollWidth/scrollHeight here. The absolute canvas and detached
    // physics objects can contribute to document overflow, which fed their own
    // size back into the world on every resize and made the scene grow.
    ...
    return {
      w:viewportW,
      h:Math.max(window.innerHeight || 0, pageBottom + 24)
    };
  }
